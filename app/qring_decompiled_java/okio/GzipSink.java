...
public final class GzipSink implements okio.Sink {
    ...
    private final okio.buffer sink;

    public GzipSink(okio.Sink sink) {
        kotlin.jvm.internal.Intrinsics.checkNotNullParameter(sink, "sink");
        okio.buffer bufferVar = new okio.buffer(sink);
        this.sink = bufferVar;
        java.util.zip.Deflater deflater = new java.util.zip.Deflater(-1, true);
        this.deflater = deflater;
        this.deflaterSink = new okio.DeflaterSink((okio.BufferedSink) bufferVar, deflater);
        this.crc = new java.util.zip.CRC32();
        okio.Buffer buffer = bufferVar.bufferField;

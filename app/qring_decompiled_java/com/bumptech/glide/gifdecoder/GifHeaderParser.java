...
public class GifHeaderParser {
    ...
    public com.bumptech.glide.gifdecoder.GifHeaderParser setData(java.nio.ByteBuffer byteBuffer) {
        reset();
    ...
    public com.bumptech.glide.gifdecoder.GifHeaderParser setData(byte[] bArr) {
        if (bArr != null) {
    ...
    private void readLSD() {
        ...
        this.header.gctSize = (int) java.lang.Math.pow(2.0d, (r0 & 7) + 1);
    ...
    private int[] readColorTable(int i) {
        ...
        try {
            ...
            if (android.util.Log.isLoggable(TAG, 3)) {
                android.util.Log.d(TAG, "Format Error Reading Color Table", e);
            }
            this.header.status = 1;
        }
        return iArr;
    ...
    private void readBlock() {
        ...
        while (true) {
            try {
                ...
                if (android.util.Log.isLoggable(TAG, 3)) {
                    android.util.Log.d(TAG, "Error Reading Block n: " + i + " count: " + i2 + " blockSize: " + this.blockSize, e);
                }
                this.header.status = 1;
                return;
            }
